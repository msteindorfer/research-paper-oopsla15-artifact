 ...
 *  @version 2.8
 *  @since   2.3
 *  @define Coll `immutable.HashSet_LazyHashCode`
 *  @define coll immutable hash set
 */
@SerialVersionUID(2L)
@deprecatedInheritance("The implementation details of immutable hash sets make inheriting from them unwise.", "2.11.0")
class HashSet_LazyHashCode[A] extends AbstractSet[A]
                    with Set[A]
                    with GenericSetTemplate[A, HashSet_LazyHashCode]
                    with SetLike[A, HashSet_LazyHashCode[A]]
                    with CustomParallelizable[A, ParHashSet[A]]
                    with Serializable
{
  import HashSet_LazyHashCode.{nullToEmpty, bufferSize, LeafHashSet_LazyHashCode}

  override lazy val hashCode: Int = super.hashCode()

  override def companion: GenericCompanion[HashSet_LazyHashCode] = HashSet_LazyHashCode

  //class HashSet_LazyHashCode[A] extends Set[A] with SetLike[A, HashSet_LazyHashCode[A]] {

  // msteindorfer: removed specialization (because I didn't transitively clone ParHashSet)
  // override def par = ParHashSet.fromTrie(this)

  override def size: Int = 0

  override def empty = HashSet_LazyHashCode.empty[A]
  ...
  override def subsetOf(that: GenSet[A]) = that match {
    case that:HashSet_LazyHashCode[A] =>
  ...
  /**
   * A specialized implementation of subsetOf for when both this and that are HashSet_LazyHashCode[A] and we can take advantage
   * of the tree structure of both operands and the precalculated hashcodes of the HashSet_LazyHashCode1 instances.
   * @param that the other set
   * @param level the level of this and that HashSet_LazyHashCode
   ...
   *              The value of level is 0 for a top-level HashSet_LazyHashCode and grows in increments of 5
   * @return true if all elements of this set are contained in that set
   */
  protected def subsetOf0(that: HashSet_LazyHashCode[A], level: Int) = {
    // The default implementation is for the empty set and returns true because the empty set is a subset of all sets
    true
  }

  override def + (e: A): HashSet_LazyHashCode[A] = updated0(e, computeHash(e), 0)

  override def + (elem1: A, elem2: A, elems: A*): HashSet_LazyHashCode[A] =
    this + elem1 + elem2 ++ elems

  override def union(that: GenSet[A]): HashSet_LazyHashCode[A] = that match {
    case that: HashSet_LazyHashCode[A] =>
      val buffer = new Array[HashSet_LazyHashCode[A]](bufferSize(this.size + that.size))
      nullToEmpty(union0(that, 0, buffer, 0))
    case _ => super.union(that)
  }

  override def intersect(that: GenSet[A]): HashSet_LazyHashCode[A] = that match {
    case that: HashSet_LazyHashCode[A] =>
      val buffer = new Array[HashSet_LazyHashCode[A]](bufferSize(this.size min that.size))
      nullToEmpty(intersect0(that, 0, buffer, 0))
    case _ => super.intersect(that)
  }

  override def diff(that: GenSet[A]): HashSet_LazyHashCode[A] = that match {
    case that: HashSet_LazyHashCode[A] =>
      val buffer = new Array[HashSet_LazyHashCode[A]](bufferSize(this.size))
      nullToEmpty(diff0(that, 0, buffer, 0))
    case _ => super.diff(that)
  }

  /**
   * Union with a leaf HashSet_LazyHashCode at a given level.
   * @param that a leaf HashSet_LazyHashCode
   ...
   *         HashSet_LazyHashCode but needs to be stored at the correct depth
   */
  private[immutable] def union0(that: LeafHashSet_LazyHashCode[A], level: Int): HashSet_LazyHashCode[A] = {
    // the default implementation is for the empty set, so we just return that
    that
  }

  /**
   * Union with a HashSet_LazyHashCode at a given level
   * @param that a HashSet_LazyHashCode
   ...
   *         HashSet_LazyHashCode but needs to be stored at the correct depth
   */
  private[immutable] def union0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = {
    // the default implementation is for the empty set, so we just return that
    that
  }
   ...
   *         self-contained HashSet_LazyHashCode but needs to be stored at the correct depth
   */
  private[immutable] def intersect0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = {
   ...
   *         self-contained HashSet_LazyHashCode but needs to be stored at the correct depth
   */
  private[immutable] def diff0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = {
    // the default implementation is for the empty set, so we just return the empty set
    null
  }

  def - (e: A): HashSet_LazyHashCode[A] =
  ...
  override def filter(p: A => Boolean) = {
    val buffer = new Array[HashSet_LazyHashCode[A]](bufferSize(size))
  ...
  protected def filter0(p: A => Boolean, negate: Boolean, level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = null
  ...
  def updated0(key: A, hash: Int, level: Int): HashSet_LazyHashCode[A] =
    new HashSet_LazyHashCode.HashSet_LazyHashCode1(key, hash)

  protected def removed0(key: A, hash: Int, level: Int): HashSet_LazyHashCode[A] = this

  protected def writeReplace(): AnyRef = new HashSet_LazyHashCode.SerializationProxy(this)
 ...
 */
object HashSet_LazyHashCode extends ImmutableSetFactory[HashSet_LazyHashCode] {

  /** $setCanBuildFromInfo */
  implicit def canBuildFrom[A]: CanBuildFrom[Coll, A, HashSet_LazyHashCode[A]] = setCanBuildFrom[A]

  private object EmptyHashSet_LazyHashCode extends HashSet_LazyHashCode[Any] { }
  private[collection] def emptyInstance: HashSet_LazyHashCode[Any] = EmptyHashSet_LazyHashCode
  
  // utility method to create a HashTrieSet from two leaf HashSet_LazyHashCodes (HashSet_LazyHashCode1 or HashSet_LazyHashCodeCollision1) with non-colliding hash code)
  private def makeHashTrieSet[A](hash0:Int, elem0:HashSet_LazyHashCode[A], hash1:Int, elem1:HashSet_LazyHashCode[A], level:Int) : HashTrieSet[A] = {
    val index0 = (hash0 >>> level) & 0x1f
    val index1 = (hash1 >>> level) & 0x1f
    if(index0 != index1) {
      val bitmap = (1 << index0) | (1 << index1)
      val elems = new Array[HashSet_LazyHashCode[A]](2)
      if(index0 < index1) {
        elems(0) = elem0
        elems(1) = elem1
      } else {
      ...
      val elems = new Array[HashSet_LazyHashCode[A]](1)
      val bitmap = (1 << index0)
  ...
  /**
   * Common superclass of HashSet_LazyHashCode1 and HashSet_LazyHashCodeCollision1, which are the two possible leaves of the Trie
   */
  private[HashSet_LazyHashCode] sealed abstract class LeafHashSet_LazyHashCode[A] extends HashSet_LazyHashCode[A] {
    private[HashSet_LazyHashCode] def hash:Int
  }

  class HashSet_LazyHashCode1[A](private[HashSet_LazyHashCode] val key: A, private[HashSet_LazyHashCode] val hash: Int) extends LeafHashSet_LazyHashCode[A] {
    override def size = 1

    override def get0(key: A, hash: Int, level: Int): Boolean =
      (hash == this.hash && key == this.key)

    override def subsetOf0(that: HashSet_LazyHashCode[A], level: Int) = {
      // check if that contains this.key
      // we use get0 with our key and hash at the correct level instead of calling contains,
      // which would not work since that might not be a top-level HashSet_LazyHashCode
      // and in any case would be inefficient because it would require recalculating the hash code
      that.get0(key, hash, level)
    }

    override def updated0(key: A, hash: Int, level: Int): HashSet_LazyHashCode[A] =
      if (hash == this.hash && key == this.key) this
      else {
        if (hash != this.hash) {
          makeHashTrieSet(this.hash, this, hash, new HashSet_LazyHashCode1(key, hash), level)
        } else {
          // 32-bit hash collision (rare, but not impossible)
          new HashSet_LazyHashCodeCollision1(hash, ListSet.empty + this.key + key)
        }
      }

    override private[immutable] def union0(that: LeafHashSet_LazyHashCode[A], level: Int): HashSet_LazyHashCode[A] = that match {
      ...
      case that: HashSet_LazyHashCode1[A] =>
        if (this.key == that.key) {
          this
        } else {
          // 32-bit hash collision (rare, but not impossible)
          new HashSet_LazyHashCodeCollision1[A](hash, ListSet.empty + this.key + that.key)
        }
      case that: HashSet_LazyHashCodeCollision1[A] =>
        val ks1 = that.ks + key
        // Could use eq check (faster) if ListSet was guaranteed to return itself
        if (ks1.size == that.ks.size) {
          that
        } else {
          new HashSet_LazyHashCodeCollision1[A](hash, ks1)
        }
    }

    override private[immutable] def union0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int) = {
      // switch to the Leaf version of union
      // we can exchange the arguments because union is symmetrical
      that.union0(this, level)
    }

    override private[immutable] def intersect0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] =
      if (that.get0(key, hash, level)) this else null

    override private[immutable] def diff0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] =
      if (that.get0(key, hash, level)) null else this

    override def removed0(key: A, hash: Int, level: Int): HashSet_LazyHashCode[A] =
      if (hash == this.hash && key == this.key) null else this

    override protected def filter0(p: A => Boolean, negate: Boolean, level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] =
  ...
  private[immutable] class HashSet_LazyHashCodeCollision1[A](private[HashSet_LazyHashCode] val hash: Int, val ks: ListSet[A]) extends LeafHashSet_LazyHashCode[A] {
    ...
    override def updated0(key: A, hash: Int, level: Int): HashSet_LazyHashCode[A] =
      if (hash == this.hash) new HashSet_LazyHashCodeCollision1(hash, ks + key)
      else makeHashTrieSet(this.hash, this, hash, new HashSet_LazyHashCode1(key, hash), level)

    override def union0(that: LeafHashSet_LazyHashCode[A], level: Int): HashSet_LazyHashCode[A] = that match {
      case that if that.hash != this.hash =>
        // different hash code, so there is no need to investigate further.
        // Just create a branch node containing the two.
        makeHashTrieSet(this.hash, this, that.hash, that, level)
      case that: HashSet_LazyHashCode1[A] =>
        ...
        if (ks1.size == ks.size) {
          ...
          // create a new HashSet_LazyHashCodeCollision with the existing hash
    ...
    override def union0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = that match {
      case that: LeafHashSet_LazyHashCode[A] =>
    ...
    override private[immutable] def intersect0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = {
      ...
      ks1.size match {
        ...
        case size if size == that.size =>
          ...
          that
        case 1 =>
          // create a new HashSet_LazyHashCode1 with the hash we already know
          new HashSet_LazyHashCode1(ks1.head, hash)
        case _ =>
          // create a new HashSet_LazyHashCodeCollision with the hash we already know and the new keys
          new HashSet_LazyHashCodeCollision1(hash, ks1)
      }
    }
    ...
    private def writeObject(out: java.io.ObjectOutputStream) {
      ...
      sys.error("cannot serialize an immutable.HashSet_LazyHashCode where all items have the same 32-bit hash code")
      //out.writeObject(kvs)
    }

    private def readObject(in: java.io.ObjectInputStream) {
      sys.error("cannot deserialize an immutable.HashSet_LazyHashCode where all items have the same 32-bit hash code")
   ...
   * children:        ---b----------------a-----------
   */
  class HashTrieSet[A](private val bitmap: Int, private[collection] val elems: Array[HashSet_LazyHashCode[A]], private val size0: Int)
        extends HashSet_LazyHashCode[A] {
    ...
    override def updated0(key: A, hash: Int, level: Int): HashSet_LazyHashCode[A] = {
      ...
      if ((bitmap & mask) != 0) {
        ...
        if (sub eq subNew) this
          ...
          val elemsNew = new Array[HashSet_LazyHashCode[A]](elems.length)
          Array.copy(elems, 0, elemsNew, 0, elems.length)
          elemsNew(offset) = subNew
          new HashTrieSet(bitmap, elemsNew, size + (subNew.size - sub.size))
        }
      } else {
        val elemsNew = new Array[HashSet_LazyHashCode[A]](elems.length + 1)
        Array.copy(elems, 0, elemsNew, 0, offset)
        elemsNew(offset) = new HashSet_LazyHashCode1(key, hash)
    ...
    override private[immutable] def union0(that: LeafHashSet_LazyHashCode[A], level: Int): HashSet_LazyHashCode[A] = {
      ...
      if ((bitmap & mask) != 0) {
        ...
        if (sub eq sub1) this
          ...
          val elems1 = new Array[HashSet_LazyHashCode[A]](elems.length)
          Array.copy(elems, 0, elems1, 0, elems.length)
          elems1(offset) = sub1
          new HashTrieSet(bitmap, elems1, size + (sub1.size - sub.size))
        }
      } else {
        val elems1 = new Array[HashSet_LazyHashCode[A]](elems.length + 1)
    ...
    override private[immutable] def union0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = that match {
      ...
      case that: HashTrieSet[A] =>
        ...
        if (rs == this.size) {
          ...
          val elems = new Array[HashSet_LazyHashCode[A]](length)
    ...
    override private[immutable] def intersect0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = that match {
      case that if that eq this =>
        // shortcut for when that is this
        // this happens often for nodes deeper in the tree, especially when that and this share a common "heritage"
        // e.g. you have a large set A and do some small operations (adding and removing elements) to it to create B
    ...
    override private[immutable] def diff0(that: HashSet_LazyHashCode[A], level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = that match {
      case that if that eq this =>
        // shortcut for when that is this
        // this happens often for nodes deeper in the tree, especially when that and this share a common "heritage"
        // e.g. you have a large set A and do some small operations (adding and removing elements) to it to create B
      ...
      case that: HashSet_LazyHashCodeCollision1[A] =>
        ...
        // to be removed
        @tailrec def removeAll(s:HashSet_LazyHashCode[A], r:ListSet[A]) : HashSet_LazyHashCode[A] =
      ...
      if ((bitmap & mask) != 0) {
        ...
        if (sub eq subNew) this
          ...
          if (bitmapNew != 0) {
            ...
            // HashSet_LazyHashCode1 or HashSet_LazyHashCodeCollision1, return the child instead
    ...
    override def subsetOf0(that: HashSet_LazyHashCode[A], level: Int): Boolean = if (that eq this) true else that match {
      ...
      case _ =>
        ...
        // if the other set is a HashSet_LazyHashCode1, we can not be a subset of it because we are a HashTrieSet with at least two children (see assertion)
        // if the other set is a HashSet_LazyHashCodeCollision1, we can not be a subset of it because we are a HashTrieSet with at least two different hash codes
    ...
    override protected def filter0(p: A => Boolean, negate: Boolean, level: Int, buffer: Array[HashSet_LazyHashCode[A]], offset0: Int): HashSet_LazyHashCode[A] = {
      ...
      if (offset == offset0) {
        ...
        val length = offset - offset0
        val elems1 = new Array[HashSet_LazyHashCode[A]](length)
    ...
    override def iterator = new TrieIterator[A](elems.asInstanceOf[Array[Iterable[A]]]) {
      final override def getElem(cc: AnyRef): A = cc.asInstanceOf[HashSet_LazyHashCode1[A]].key
    }
   ...
   */
  @inline private def nullToEmpty[A](s: HashSet_LazyHashCode[A]): HashSet_LazyHashCode[A] = if (s eq null) empty[A] else s
  ...
  @SerialVersionUID(2L) private class SerializationProxy[A,B](@transient private var orig: HashSet_LazyHashCode[A]) extends Serializable {
    private def writeObject(out: java.io.ObjectOutputStream) {
      val s = orig.size
      out.writeInt(s)
      for (e <- orig) {
