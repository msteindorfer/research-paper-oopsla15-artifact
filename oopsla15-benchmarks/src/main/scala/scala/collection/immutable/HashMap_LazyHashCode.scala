 ...
 *  @define Coll `immutable.HashMap_LazyHashCode`
...
class HashMap_LazyHashCode[A, +B] extends AbstractMap[A, B]
                        with Map[A, B]
                        with MapLike[A, B, HashMap_LazyHashCode[A, B]]
                        with Serializable
                        with CustomParallelizable[(A, B), ParHashMap[A, B]]
{
  import HashMap_LazyHashCode.{nullToEmpty, bufferSize}

  override lazy val hashCode: Int = super.hashCode()

  override def size: Int = 0

  override def empty = HashMap_LazyHashCode.empty[A, B]
  ...
  override def updated [B1 >: B] (key: A, value: B1): HashMap_LazyHashCode[A, B1] =
    updated0(key, computeHash(key), 0, value, null, null)

  override def + [B1 >: B] (kv: (A, B1)): HashMap_LazyHashCode[A, B1] =
    updated0(kv._1, computeHash(kv._1), 0, kv._2, kv, null)

  override def + [B1 >: B] (elem1: (A, B1), elem2: (A, B1), elems: (A, B1) *): HashMap_LazyHashCode[A, B1] =
    this + elem1 + elem2 ++ elems

  def - (key: A): HashMap_LazyHashCode[A, B] =
    removed0(key, computeHash(key), 0)

  override def filter(p: ((A, B)) => Boolean) = {
    val buffer = new Array[HashMap_LazyHashCode[A, B]](bufferSize(size))
  ...
  protected def filter0(p: ((A, B)) => Boolean, negate: Boolean, level: Int, buffer: Array[HashMap_LazyHashCode[A, B @uV]], offset0: Int): HashMap_LazyHashCode[A, B] = null
  ...
  import HashMap_LazyHashCode.{Merger, MergeFunction, liftMerger}

  private[collection] def get0(key: A, hash: Int, level: Int): Option[B] = None

  private[collection] def updated0[B1 >: B](key: A, hash: Int, level: Int, value: B1, kv: (A, B1), merger: Merger[A, B1]): HashMap_LazyHashCode[A, B1] =
    new HashMap_LazyHashCode.HashMap_LazyHashCode1(key, hash, value, kv)

  protected def removed0(key: A, hash: Int, level: Int): HashMap_LazyHashCode[A, B] = this

  protected def writeReplace(): AnyRef = new HashMap_LazyHashCode.SerializationProxy(this)

  def split: Seq[HashMap_LazyHashCode[A, B]] = Seq(this)
   ...
   */
  def merged[B1 >: B](that: HashMap_LazyHashCode[A, B1])(mergef: MergeFunction[A, B1]): HashMap_LazyHashCode[A, B1] = merge0(that, 0, liftMerger(mergef))

  protected def merge0[B1 >: B](that: HashMap_LazyHashCode[A, B1], level: Int, merger: Merger[A, B1]): HashMap_LazyHashCode[A, B1] = that

  // msteindorfer: removed specialization (because I didn't transitively clone ParHashMap)
 ...
 *  @since   2.3
 */
object HashMap_LazyHashCode extends ImmutableMapFactory[HashMap_LazyHashCode] with BitOperations.Int {
  ...
  /** $mapCanBuildFromInfo */
  implicit def canBuildFrom[A, B]: CanBuildFrom[Coll, (A, B), HashMap_LazyHashCode[A, B]] = new MapCanBuildFrom[A, B]
  def empty[A, B]: HashMap_LazyHashCode[A, B] = EmptyHashMap_LazyHashCode.asInstanceOf[HashMap_LazyHashCode[A, B]]

  private object EmptyHashMap_LazyHashCode extends HashMap_LazyHashCode[Any, Nothing] { }

  // utility method to create a HashTrieMap from two leaf HashMap_LazyHashCodes (HashMap_LazyHashCode1 or HashMap_LazyHashCodeCollision1) with non-colliding hash code)
  private def makeHashTrieMap[A, B](hash0:Int, elem0:HashMap_LazyHashCode[A, B], hash1:Int, elem1:HashMap_LazyHashCode[A, B], level:Int, size:Int) : HashTrieMap[A, B] = {
    val index0 = (hash0 >>> level) & 0x1f
    val index1 = (hash1 >>> level) & 0x1f
    if(index0 != index1) {
      val bitmap = (1 << index0) | (1 << index1)
      val elems = new Array[HashMap_LazyHashCode[A,B]](2)
      if(index0 < index1) {
        elems(0) = elem0
        elems(1) = elem1
      } else {
      ...
      val elems = new Array[HashMap_LazyHashCode[A,B]](1)
      val bitmap = (1 << index0)
  ...
  class HashMap_LazyHashCode1[A,+B](private[collection] val key: A, private[collection] val hash: Int, private[collection] val value: (B @uV), private[collection] var kv: (A,B @uV)) extends HashMap_LazyHashCode[A,B] {
    override def size = 1
    ...
    private[collection] override def updated0[B1 >: B](key: A, hash: Int, level: Int, value: B1, kv: (A, B1), merger: Merger[A, B1]): HashMap_LazyHashCode[A, B1] =
      if (hash == this.hash && key == this.key ) {
        if (merger eq null) {
          if (this.value.asInstanceOf[AnyRef] eq value.asInstanceOf[AnyRef]) this
          else new HashMap_LazyHashCode1(key, hash, value, kv)
        } else {
          val nkv = merger(this.kv, kv)
          new HashMap_LazyHashCode1(nkv._1, hash, nkv._2, nkv)
        }
      } else {
        if (hash != this.hash) {
          // they have different hashes, but may collide at this level - find a level at which they don't
          val that = new HashMap_LazyHashCode1[A, B1](key, hash, value, kv)
          makeHashTrieMap[A,B1](this.hash, this, hash, that, level, 2)
        } else {
          // 32-bit hash collision (rare, but not impossible)
          new HashMap_LazyHashCodeCollision1(hash, ListMap.empty.updated(this.key,this.value).updated(key,value))
        }
      }

    override def removed0(key: A, hash: Int, level: Int): HashMap_LazyHashCode[A, B] =
      if (hash == this.hash && key == this.key) HashMap_LazyHashCode.empty[A,B] else this

    override protected def filter0(p: ((A, B)) => Boolean, negate: Boolean, level: Int, buffer: Array[HashMap_LazyHashCode[A, B @uV]], offset0: Int): HashMap_LazyHashCode[A, B] =
    ...
    private[HashMap_LazyHashCode] def ensurePair: (A,B) = if (kv ne null) kv else { kv = (key, value); kv }
    protected override def merge0[B1 >: B](that: HashMap_LazyHashCode[A, B1], level: Int, merger: Merger[A, B1]): HashMap_LazyHashCode[A, B1] = {
      that.updated0(key, hash, level, value, kv, merger.invert)
    }
  }

  private[collection] class HashMap_LazyHashCodeCollision1[A, +B](private[collection] val hash: Int, val kvs: ListMap[A, B @uV])
          extends HashMap_LazyHashCode[A, B @uV] {
    // assert(kvs.size > 1)
    ...
    private[collection] override def updated0[B1 >: B](key: A, hash: Int, level: Int, value: B1, kv: (A, B1), merger: Merger[A, B1]): HashMap_LazyHashCode[A, B1] =
      if (hash == this.hash) {
        if ((merger eq null) || !kvs.contains(key)) new HashMap_LazyHashCodeCollision1(hash, kvs.updated(key, value))
        else new HashMap_LazyHashCodeCollision1(hash, kvs + merger((key, kvs(key)), kv))
      } else {
        val that = new HashMap_LazyHashCode1(key, hash, value, kv)
    ...
    override def removed0(key: A, hash: Int, level: Int): HashMap_LazyHashCode[A, B] =
      if (hash == this.hash) {
        ...
        kvs1.size match {
          case 0 =>
            HashMap_LazyHashCode.empty[A,B]
          case 1 =>
            val kv = kvs1.head
            new HashMap_LazyHashCode1(kv._1,hash,kv._2,kv)
          case x if x == kvs.size =>
            this
          case _ =>
            new HashMap_LazyHashCodeCollision1(hash, kvs1)
        }
      } else this
    ...
    override protected def filter0(p: ((A, B)) => Boolean, negate: Boolean, level: Int, buffer: Array[HashMap_LazyHashCode[A, B @uV]], offset0: Int): HashMap_LazyHashCode[A, B] = {
      ...
      kvs1.size match {
        ...
        case 1 =>
          ...
          new HashMap_LazyHashCode1(k, hash, v, kv)
        case x if x == kvs.size =>
          this
        case _ =>
          new HashMap_LazyHashCodeCollision1(hash, kvs1)
    ...
    override def split: Seq[HashMap_LazyHashCode[A, B]] = {
      val (x, y) = kvs.splitAt(kvs.size / 2)
      def newhm(lm: ListMap[A, B @uV]) = new HashMap_LazyHashCodeCollision1(hash, lm)
  ...
  class HashTrieMap[A, +B](
    ...
    private[collection] val elems: Array[HashMap_LazyHashCode[A, B @uV]],
    ...
    private[collection] override def updated0[B1 >: B](key: A, hash: Int, level: Int, value: B1, kv: (A, B1), merger: Merger[A, B1]): HashMap_LazyHashCode[A, B1] = {
      ...
      if ((bitmap & mask) != 0) {
        ...
        if(subNew eq sub) this else {
          val elemsNew = new Array[HashMap_LazyHashCode[A,B1]](elems.length)
          Array.copy(elems, 0, elemsNew, 0, elems.length)
          elemsNew(offset) = subNew
          new HashTrieMap(bitmap, elemsNew, size + (subNew.size - sub.size))
        }
      } else {
        val elemsNew = new Array[HashMap_LazyHashCode[A,B1]](elems.length + 1)
        Array.copy(elems, 0, elemsNew, 0, offset)
        elemsNew(offset) = new HashMap_LazyHashCode1(key, hash, value, kv)
    ...
    override def removed0(key: A, hash: Int, level: Int): HashMap_LazyHashCode[A, B] = {
      ...
      if ((bitmap & mask) != 0) {
        ...
        if (subNew eq sub) this
          ...
          if (bitmapNew != 0) {
            val elemsNew = new Array[HashMap_LazyHashCode[A,B]](elems.length - 1)
            Array.copy(elems, 0, elemsNew, 0, offset)
          ...
          val elemsNew = new Array[HashMap_LazyHashCode[A,B]](elems.length)
          Array.copy(elems, 0, elemsNew, 0, elems.length)
          elemsNew(offset) = subNew
    ...
    override protected def filter0(p: ((A, B)) => Boolean, negate: Boolean, level: Int, buffer: Array[HashMap_LazyHashCode[A, B @uV]], offset0: Int): HashMap_LazyHashCode[A, B] = {
      ...
      if (offset == offset0) {
        ...
        val elems1 = new Array[HashMap_LazyHashCode[A, B]](length)
    ...
    override def iterator: Iterator[(A, B)] = new TrieIterator[(A, B)](elems.asInstanceOf[Array[Iterable[(A, B)]]]) {
      final override def getElem(cc: AnyRef): (A, B) = cc.asInstanceOf[HashMap_LazyHashCode1[A, B]].ensurePair
    }
    ...
    override def split: Seq[HashMap_LazyHashCode[A, B]] = if (size == 1) Seq(this) else {
    ...
    protected override def merge0[B1 >: B](that: HashMap_LazyHashCode[A, B1], level: Int, merger: Merger[A, B1]): HashMap_LazyHashCode[A, B1] = that match {
      case hm: HashMap_LazyHashCode1[_, _] =>
      ...
      case hm: HashTrieMap[_, _] =>
        ...
        val merged = new Array[HashMap_LazyHashCode[A, B1]](subcount)
      ...
      case hm: HashMap_LazyHashCodeCollision1[_, _] => that.merge0(this, level, merger.invert)
      case hm: HashMap_LazyHashCode[_, _] => this
   ...
   */
  @inline private def nullToEmpty[A, B](m: HashMap_LazyHashCode[A, B]): HashMap_LazyHashCode[A, B] = if (m eq null) empty[A, B] else m
  ...
  @SerialVersionUID(2L)
  private class SerializationProxy[A,B](@transient private var orig: HashMap_LazyHashCode[A, B]) extends Serializable {
